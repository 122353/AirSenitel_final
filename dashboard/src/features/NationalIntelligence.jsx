...
export default function NationalIntelligence() {
  ...
  return <div className="page national-page">
    <PageHeading eyebrow="INDIA / MULTI-SCALE EARLY WARNING" title="From a national signal to a local investigation." description="Search any Indian city, village or postcode. VayuNirikshak uses the best available evidence without pretending a 45 km model grid is a street-level monitor." />
    ...
    <div className="evidence-boundary"><ShieldCheck size={18} /><div><strong>What VayuNirikshak can and cannot do.</strong><p>It can screen every valid India coordinate with a regional atmospheric model, discover stations from the configured India-wide feed, and run station-specific forecasts when sufficient history exists. It cannot produce microscopic AQI where no local surface instruments exist. Satellite aerosol, weather and citizen reports improve context; calibrated ground sensors remain necessary for neighbourhood claims.</p></div></div>
  </div>;
}
